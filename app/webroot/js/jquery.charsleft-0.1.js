...
(function($){
	$.fn.charsLeft = function(customOptions){
		...
		return this.each(function(i){
			...
			charHTML += "</div>";*/
            var charHTML = "<span class='" + options.countClass +"'>";
            charHTML += options.charPrefix + "&nbsp;" + options.maxChars + "&nbsp;" + options.charSuffix;
            charHTML += "</span>";
			//Attach our HTML
			...
			$this.bind("keyup.charsLeft", function(){
                ...
                if (messageCharsLeft < 0){
                    $this.val($this.val().substring(0, options.maxChars));
                }
                else{
                    var newMessager = options.charPrefix + " " + messageCharsLeft + " " + options.charSuffix;
                    $charCount.text(newMessager);    
                }
