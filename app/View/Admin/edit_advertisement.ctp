...
<article class="module width_full">
    <header><h3>Thêm quản cáo</h3></header>
        <div class="module_content">
            <form id="frm_advertisement" name="frm_advertisement" method="post" action="<?php echo $this->Html->url(array('controller' => 'Admin', 'action' =>'editAdvertisement', $data['id'])) ?>" class="login-subscription" enctype="multipart/form-data">
                <input type="hidden" name="data[Advertisement][id]"  value="<?php echo $data['id']; ?>"/>
                <fieldset style="width:48%; float:left; margin-right: 3%;"> <!-- to make two field float next to one another, adjust values accordingly -->
                    <label>Thông tin *</label>
                    <input type="text" class="validate[required]" style="width:92%;" value="<?php echo isset($data['information']) ? $data['information'] : '' ?>" id="infomation" name="data[Advertisement][information]" value="<?php echo isset($data['information']) ? $data['information'] : '' ?>"  id="information" />
                </fieldset>
                ...
                <fieldset> <!-- to make two field float next to one another, adjust values accordingly -->
                    ...
                    <input  style="width:92%;"  type="text" class="validate[required]" name="data[Advertisement][url]" value="<?php echo isset($data['url']) ? $data['url'] : '' ?>"  id="FileImage" />
                </fieldset>
                <fieldset> <!-- to make two field float next to one another, adjust values accordingly -->
                    <label>Banner</label>
                    <br />
                    <br />
                    <img alt="" style="float:left;" src="<?php echo $this->Html->url('/') .  (isset($data['banner_url']) ? $data['banner_url'] : NO_IMG_URL)  ?>" />
