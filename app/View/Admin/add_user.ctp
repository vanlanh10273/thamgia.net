...
<article class="module width_full">
    <header><h3>Thêm user</h3></header>
        <div class="module_content">
            <form id="frm_user" name="frm_user" method="post" action="<?php echo $this->Html->url(array('controller' => 'Admin', 'action' =>'addUser')) ?>" class="login-subscription" enctype="multipart/form-data">
                <fieldset style="width:48%; float:left; margin-right: 3%;"> <!-- to make two field float next to one another, adjust values accordingly -->
                    <label>email *</label>
                    <input type="text" class="validate[required]" style="width:92%;" value="<?php echo isset($data['email']) ? $data['email'] : '' ?>" id="email" name="data[User][email]"  />
                </fieldset>
                ...
                <span>Mật khẩu mặc định là: <?php echo DEFAULT_PASSWORD; ?></span>
                <div class="clear"></div>
            </form>
        </div>
    <footer>
